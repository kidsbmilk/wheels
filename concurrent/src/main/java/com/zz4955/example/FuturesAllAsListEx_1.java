 ...
 * 在这个例子中，三个future都出现异常了，这三个异常是相同的，只有第一次会记录在seenExceptions里，只有一个future存在第一次记录异常，所以，
 * 不打印"Got more than one input Future failure. Logging failures after the first"的记录。
 * 具体代码见RunningState.init方法。
 */
public class FuturesAllAsListEx_1 {
